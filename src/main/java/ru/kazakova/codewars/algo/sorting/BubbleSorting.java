...
public class BubbleSorting {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(Arrays.toString(bubbleSorting2(new int[]{8, 3, 0, 12, 9, 5, 7})));
    }
    ...
    public static int[] bubbleSorting2(int[] ints) {
        if (ints.length == 0 || ints.length == 1) {
            return ints;
        }

        // пройтись по массиву
        // переставить те элем которые меньше левее
        // зациклить прохождение
        boolean isSorted = false;
        while (!isSorted) {
            isSorted = true;
            for (int i = 1; i < ints.length; i++) {
